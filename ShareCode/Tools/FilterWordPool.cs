    ...
    class FilterWord : IDisposable
    {
        ...
        private string ToDBC(string input)
        {
            unsafe
            {
                ...
                string result = Saker.ToolsMethods.FastAllocateString(len);
                fixed (char* c = input)
                fixed (char* r = result)
                {
                    for (int i = 0; i < len; i++)
...
";
    ...
    public class FilterWordPool : IDisposable
    {
        ...
        static Saker.Tools.MessageQueueMultiple<AsyncFilterWordTask> _task = new MessageQueueMultiple<AsyncFilterWordTask>();
        ...
        public string Filter(string sourctText, char replaceChar)
        {
            ...
            {
                Saker.SystemErrorProvide.OnSystemErrorHandleEvent(this, ex);
                return "";
            }
            finally
            {
