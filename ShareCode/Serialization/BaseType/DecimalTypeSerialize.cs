...
namespace Saker.Serialization.BaseType
{
    internal class DecimalTypeSerialize : TypeSerializationBase<decimal>
    {
        ...
        {
           Saker.Serialization. BigEndian.BigEndianPrimitiveTypeSerializer.Instance.WriteValue(stream, obj);
        }
        public static decimal Read(System.IO.Stream stream)
        {
            decimal value;
            Saker.Serialization.BigEndian.BigEndianPrimitiveTypeSerializer.Instance.ReadValue(stream, out value);
            return value;
        }
