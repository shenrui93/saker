    ...
    internal class ByteArrayTypeSerialize : TypeSerializationBase<byte[]>
    {
        ...
        public static void Write(byte[] obj, System.IO.Stream stream)
        {
            ...
            else if (length < ushort.MaxValue)
            {
                ...
                Saker.Serialization.BigEndian.BigEndianPrimitiveTypeSerializer.Instance.WriteValue(stream, (ushort)length);
            }
            else
            {
                stream.WriteByte(255);
                Saker.Serialization.BigEndian.BigEndianPrimitiveTypeSerializer.Instance.WriteValue(stream, length);
        ...
        public static byte[] Read(System.IO.Stream stream)
        {
            ...
            {
                ushort t;
                Saker.Serialization.BigEndian.BigEndianPrimitiveTypeSerializer.Instance.ReadValue(stream, out t);
                length = t;
            }
            else if (length == 255)
            {
                Saker.Serialization.BigEndian.BigEndianPrimitiveTypeSerializer.Instance. ReadValue(stream, out length);
            }
