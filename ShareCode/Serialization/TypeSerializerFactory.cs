...
using Saker.Serialization.BaseType;
using Saker.Serialization.DynmaicType;
...
namespace Saker.Serialization
{
    ...
    internal static class TypeSerializerFactory
    {
        ...
        static void TryCreateTypeSerializer(Type type)
        {
            ...
            {
                Saker.SystemErrorProvide.OnSystemErrorHandleEvent(null, new System.Exception($"处理序列化类型：{type.FullName} 出现异常", ex));
#if TRACE
                throw ex; 
#endif
            }
