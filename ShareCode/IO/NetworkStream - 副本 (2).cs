    ...
    public class NetworkStream : Stream
    {
        ...
        private void WriteInternal(byte[] buffer, int offset, int count)
        {
            lock (root)
            {
                ...
                if (newcount > cat)
                {
                    ...
                    System.Buffer.BlockCopy(_buffer, 0, newBuffer, 0, this.count);
                    this._buffer = newBuffer;
                }
                Buffer.BlockCopy(buffer, offset, this._buffer, this.count, count);
                this.count = newcount;
            }
        }
        private int ReadInternal(byte[] buffer, int offset, int count)
        {
            lock (root)
            {
                ...
                System.Buffer.BlockCopy(this._buffer, 0, buffer, offset, relCount);
                var newcount = this.count - relCount;
                if (newcount != 0)
                {
                    System.Buffer.BlockCopy(this._buffer, relCount, this._buffer, 0, newcount);
                }
                this.count = newcount;
                InitBuffer();
                return relCount;
        ...
        private int ReadOnlyInternal(byte[] buffer, int offset, int count)
        {
            lock (root)
            {
                if (this.count == 0) return 0;
                var relCount = this.count > count + offset ? count : this.count - offset;
                System.Buffer.BlockCopy(this._buffer, offset, buffer, 0, relCount);
                return relCount;
            }
        }
        private byte[] ReadBytesInternal(int offset, int count)
        {
            lock (root)
            {
                ...
                if (newcount != 0)
                {
                    Buffer.BlockCopy(this._buffer, relCount, this._buffer, 0, newcount);
                }
                this.count = newcount;
                InitBuffer();
                return buffer;
        ...
        private byte[] ReadOnlyBytesInternal(int offset, int count)
        {
            lock (root)
            {
                ...
                var relCount = this.count > count + offset ? count : this.count - offset;
                var buffer = new byte[relCount];
                Buffer.BlockCopy(this._buffer, offset, buffer, 0, relCount);
                return buffer;
            }
        }
        ...
        private void RemoveInternal(int count)
        {
            lock (root)
            {
                ...
                if (newcount > 0)
                {
                    Buffer.BlockCopy(this._buffer, count, this._buffer, 0, newcount);
                }
        ...
        private void InitBuffer()
        {
            ...
            if (this.cat >= MaxBufferCat && this.count <= MaxBufferCount)
            {
                ...
                if (this.count > 0)
                    Buffer.BlockCopy(this._buffer, 0, newbuffer, 0, this.count);
                this._buffer = newbuffer;
            }
